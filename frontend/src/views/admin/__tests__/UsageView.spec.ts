...
const { list, exportList, getStats, getSnapshotV2, getById, getModelStats, getBodyLog, showError, listErrorLogs, routeQuery, aoaToSheet, sheetAddAoa, saveAs, xlsxWrite } = vi.hoisted(() => {
  ...
  setup(props) {
    return () =>
      h(
        ...
        `${props.show ? 'open' : 'closed'}|${props.loading ? 'loading' : 'idle'}|${(props.bodyLog as Record<string, unknown> | null)?.request_id ?? ''}|${(props.bodyLog as Record<string, unknown> | null)?.response_body ?? ''}`,
      )
  },
})
...
describe('admin UsageView distribution metric toggles', () => {
  ...
  it('keeps model and group metric toggles independent without refetching chart data', async () => {
    ...
    expect(wrapper.get('[data-test="body-log-modal"]').text()).toContain('open')
		...
		expect(wrapper.get('[data-test="body-log-modal"]').text()).toContain('{"text":"ok"}')
	})
