    ...
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiAllowClaudeCodeCodexPluginDesc:
          '历史兼容选项。新版本会将此放行规则迁移到 Codex 客户端限制策略。',
        bodyLogTitle: '请求体日志',
        bodyLogDescription: '将网关转发时的请求体和响应体按上限截断后写入数据库，便于排查上游兼容性与请求差异。',
        bodyLogEnabled: '启用请求体日志',
        bodyLogEnabledHint: '关闭时不记录请求体和响应体，只保留常规使用与错误日志。',
        bodyLogMaxBytes: '单侧最大字节数',
        bodyLogMaxBytesHint: '每个请求体或响应体分别按此上限截断保存。有效范围 1 到 1048576 字节，超出部分由后端归一化。',
        bodyLogCaptureRequest: '记录请求体',
        bodyLogCaptureRequestHint: '保存发送到上游前的请求体快照。',
        bodyLogCaptureResponse: '记录响应体',
        bodyLogCaptureResponseHint: '保存上游返回给网关的响应体快照。',
