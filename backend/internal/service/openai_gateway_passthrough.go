		...
		if resp.StatusCode == http.StatusForbidden && hasOptionalOpenAIImageGenerationTool(openAIResponsesEndpoint, reqModel, body) {
			if retryBody, stripped := stripOptionalOpenAIImageGenerationTool(body); stripped {
				...
				if retryBuildErr == nil {
					...
					if retryErr == nil {
						defer func(bodyCloser io.Closer) { _ = bodyCloser.Close() }(retryResp.Body)
						body = retryBody
						resp = retryResp
					}
				}
...
func normalizeOpenAIPassthroughRequestBody(account *Account, body []byte) []byte {
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	writeLine := func(line string) error {
		...
		if n > 0 {
			...
			streamCapture.WriteString(written[:n])
		}
		return err
	}
	...
	writePendingLines := func() bool {
		for _, pending := range pendingLines {
			if err := writeLine(pending); err != nil {
