...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	captureWriter := io.MultiWriter(w, streamCapture)
	bufferedWriter := bufio.NewWriterSize(captureWriter, 4*1024)
	...
	flushBuffered := func() error {
		if firstOutputStage != nil && firstTokenMs == nil && !firstOutputStage.closed {
			if err := firstOutputStage.CommitTo(captureWriter); err != nil {
				return err
			}
		} else {
	...
	for {
		...
		case <-keepaliveCh:
			...
			if guardFirstOutput {
				...
				if _, err := captureWriter.Write([]byte(":\n\n")); err != nil {
					clientDisconnected = true
					logger.LegacyPrintf("service.openai_gateway", "Client disconnected during streaming, continuing to drain upstream for billing")
					continue
				}
