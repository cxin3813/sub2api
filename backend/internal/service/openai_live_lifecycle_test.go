...
func TestFinalizeLiveCallLeavesSessionIDNilWhenAbsent(t *testing.T) {
	record := &LiveCallRecord{
	...
	require.Len(t, usageRepo.logs, 1)
	require.Nil(t, usageRepo.logs[0].SessionID)
}
...
// TestFinalizeLiveCallStoreWriteFailureStillReleasesAndRecordsUsage locks the
// last-resort path: after an observer has exhausted Redis retries, its final
// MarkLiveCallClosed may fail for the same outage. The local record must still
// release the lease and write the idempotent usage record rather than returning
// early and losing both.
func TestFinalizeLiveCallStoreWriteFailureStillReleasesAndRecordsUsage(t *testing.T) {
	record := &LiveCallRecord{
		CallID:     "call_mark_closed_failure",
		CallHash:   hashLiveCallID("call_mark_closed_failure"),
		AccountID:  11,
		APIKeyID:   22,
		UserID:     33,
		LeaseID:    "lease-1",
	...
	store := &liveTestStore{markClosedErr: errors.New("redis: i/o timeout")}
	require.NoError(t, store.SaveLiveCall(context.Background(), record, time.Hour))
	concurrencyCache := &liveTestConcurrencyCache{}
	usageRepo := &liveTestUsageRepo{}
	svc := &OpenAIGatewayService{
