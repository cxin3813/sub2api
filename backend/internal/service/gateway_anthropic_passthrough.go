...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	retryStart := time.Now()
	lastWireBody := cloneBytes(input.Body)
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		lastWireBody = cloneBytes(wireBody)
	...
	return &ForwardResult{
		...
		RequestBody:         cloneBytes(lastWireBody),
	...
	for {
		...
		case ev, ok := <-events:
			...
			if !clientDisconnected {
				...
				if err := writeString(restored); err != nil {
...
type anthropicPassthroughNonStreamingResult struct {
	*ClaudeUsage
	usage        *ClaudeUsage
	responseBody []byte
	statusCode   int
...
func (s *GatewayService) handleNonStreamingResponseAnthropicAPIKeyPassthrough(
	...
	c *gin.Context,
	account *Account,
) (*anthropicPassthroughNonStreamingResult, error) {
	if s.rateLimitService != nil {
		s.rateLimitService.UpdateSessionWindow(ctx, account, resp.Header)
	}
	...
	return &anthropicPassthroughNonStreamingResult{
		ClaudeUsage:  usage,
		usage:        usage,
