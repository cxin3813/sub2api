...
func (s *OpenAIGatewayService) CreateLiveCall(
	...
	for attempt := 0; attempt <= 3; attempt++ {
		...
		record := &LiveCallRecord{
			...
			SessionID:             optionalTrimmedStringPtr(identity.SessionID),
...
func (s *OpenAIGatewayService) finalizeLiveCall(record *LiveCallRecord) {
	...
	cancel()
	// Redis 写入失败时不能直接返回：observer 正是因 store 故障才走到此处，
	// 若继续依赖 MarkLiveCallClosed，usage log 和并发租约会一起丢失。重复
	// finalize 是安全的，usage_logs 以 (request_id, api_key_id) 去重，租约释放
	// 也是幂等操作；因此仅在已明确非首次关闭时跳过后续落库。
	if err == nil && !first {
		return
	}
