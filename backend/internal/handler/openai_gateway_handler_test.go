...
func TestOpenAIResponsesWebSocket_PassthroughBodyLogCapturesClientResponse(t *testing.T) {
	...
	require.Len(t, got.bodyLogs, 1)
	require.Len(t, got.clientEvents, 1)
	bodyLog := got.bodyLogs[0]
	require.Equal(t, got.clientEvents[0], bodyLog.ResponseBody)
	require.Greater(t, bodyLog.ResponseBodyBytes, int64(0))
	require.Equal(t, int64(len(got.clientEvents[0])), bodyLog.ResponseBodyBytes)
	require.NotEmpty(t, bodyLog.ResponseBodySHA256)
}
